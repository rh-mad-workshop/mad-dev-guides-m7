...
** Access from your web browser our https://gitlab-gitlab.%SUBDOMAIN%[GitLab^] instance.
** Click on the Software Templates repository, *rhdhub/software-templates*.
** You will find the *showcase-templates.yaml* file. This file defines the templates shown on the Software Templates web page on Red Hat Developer Hub.
** On the file, you will see all the defined templates per each application type:
...
** Go back to the *rhdhub/software-templates* folder and click on *scaffolder-templates* and the application template: *quarkus-backend*.

image:intro/gitlab_st_details.png[width=40%]  

1 - The *template file* is the one defining your workflow when creating a component. 

* Review the file. The file is composed of parameters and steps. The parameters are defined to interact with the user through the UI workflow. Specifically, when creating a component, you will be asked to:
...
* *helm/app* folder contains all the manifests for deploy the application using *Helm Charts*. 
* *helm/pipeline* folder contains all the manifests for build the application using https://docs.openshift.com/pipelines/latest/about/understanding-openshift-pipelines.html[*OpenShift Pipelines*^]. 

=== 1.4.3 Red Hat Developer Hub Configurations

Red Hat Developer Hub is flexible and supports multiple plugins. For this workshop, we have used the current setup described in this section:

image:module4/rhdh_architecture.png[width=50%]  
